...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeValinorpaycoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeValinorpaycoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeValinorpaycoinAmount = strAnonymizeValinorpaycoinAmount.remove(strAnonymizeValinorpaycoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeValinorpaycoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeValinorpaycoinAmount * COIN) nMaxToAnonymize = nAnonymizeValinorpaycoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeValinorpaycoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeValinorpaycoinAmount));
        strAnonymizeValinorpaycoinAmount = strAnonymizeValinorpaycoinAmount.remove(strAnonymizeValinorpaycoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeValinorpaycoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeValinorpaycoinAmount == 0) {

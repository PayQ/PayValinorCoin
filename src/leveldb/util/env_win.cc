...
std::wstring GetLastErrSzW()
{
    ...
    std::wstring Err = (VPCWSTR)lpMsgBuf;
    LocalFree(lpMsgBuf);
    return Err;
}
...
BOOL Win32RandomAccessFile::_Init( VPCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
...
BOOL Win32MapFile::_Init( VPCWSTR Path )
{
...
BOOL Win32FileLock::_Init( VPCWSTR path )
{
    BOOL bRet = FALSE;
    if(!_hFile)
